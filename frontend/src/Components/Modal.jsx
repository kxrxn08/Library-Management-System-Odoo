...
const Modal = () => {
  ...
  const handleBooking = () => {
    ...
    axios.post("http://localhost:9999/api/books/addbook", formData, {
      headers: {
        'authorization': token
      }
    }).then((data) => {
    ...
    <div>
      ...
      {isOpen && (
        <div
          ...
          tabIndex="-1"
          aria-hidden="true"
          className="fixed inset-0 z-50 flex justify-center items-center w-100   max-w-screen-2xl  bg-opacity-50 modal-container"
          style={{ backgroundColor: "#e5e4f1" }}
        >
          <div className="relative p-4 w-100  max-w-2xl h-full max-h-2xl  rounded-lg shadow dark:bg-gray-700  overflow-y-auto h-100" style={{
            ...
            <div className="py-4 md:p-5 space-y-4 w-100">
              <div className=" w-full px-2   dark:bg-gray-700" style={{ backgroundColor: "#704b66" }}>
              ...
              <p className="text-base leading-relaxed text-white dark:text-gray-400">
                Add a book simply by entering the ISBN_13 Number.
              </p>
                <div className="mb-4">
                  {/* <h1 className='px-2 font-bold text-white'>You can search book by ISBN_13 Number</h1> */}
                  <div className='flex gap-5 mt-2'>
                    <label className="block text-white dark:text-gray-400 mb-2" htmlFor='isbn' ><b>ISBN_13 Number </b> </label>
                    <input type="tel" className='rounded-lg text-black' placeholder=' Add ISBN Number' name="isbn" id="isbn" onChange={(e) => { setISBNNumber(e.target.value) }} />
